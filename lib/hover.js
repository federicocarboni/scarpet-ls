 ...
 * @param {import('./builtinsData.js').BuiltinFunction} definition
 * @param {string} name
...
function getBuiltinFunctionHover(markdown, definition, name, reference) {
    ...
    contents += (markdown ? definition.markdown : definition.plain) || '';
    return {
        contents: {
            kind: MarkupKind.Markdown,
            value: contents,
        },
        range: getRange(reference.kind === 'FunctionExpression' ? reference.name : reference),
    };
}

/**
 * @param {boolean} markdown
 * @param {import('./builtinsData.js').BuiltinCallback} callback
 * @param {import('scarpet-parser').FunctionExpression} signature
 */
function getCallbackHover(markdown, callback, signature) {
    ...
    contents += ') -> ...\n';
    if (markdown) contents += '```\n\n';
    if (markdown && callback.markdown) contents += callback.markdown;
    else if (callback.plain) contents += callback.plain;
    return {
        contents: {
            kind: markdown ? MarkupKind.Markdown : MarkupKind.PlainText,
            value: contents,
        },
        range: getRange(signature.name),
    };
}
...
function getFunctionHover(markdown, definition, node) {
    ...
    return {
        ...
        range: getRange(node.kind === 'FunctionExpression' ? node.name : node),
    };
}
...
export function getHoverContents(markdown, root, node) {
    switch (node.kind) {
        case 'FunctionExpression': {
            console.log('' + (node.name.value in data.callbacks));
            if (node.name.value in data.callbacks
            ) {
                return getCallbackHover(
                    markdown,
                    data.callbacks[node.name.value],
                    node,
                );
            }
            if (node.name.value in data.functions)
                return getBuiltinFunctionHover(
                    markdown,
                    data.functions[node.name.value],
                    node.name.value,
                    node,
                );
        ...
        case 'Constant':
            if (node.name in data.constants)
                return {
                    contents: {
                        kind: MarkupKind.Markdown,
                        value:
