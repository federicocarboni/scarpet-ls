...
// const constCompletionItems = Object.entries(data.constants).map(
//     ([key, value]) => ({
//         label: key,
...
// const funcCompletionItems = Object.entries(data.functions).map(
//     ([key, value]) => ({
//         label: key,
...
// const callbackCompletionItems = Object.entries(data.callbacks).map(
//     ([key, value]) => ({
//         label: key,
//         kind: CompletionItemKind.Function,
//         documentation: value.markdown
...
//         deprecated: !!value.deprecated,
//         textEditText: `${key}(${value.params.map((param, i) => `\${${i + 1}:${param.name}}`).join(', ')}) -> ($0);`,
//     }),
// );
...
export function initializeDefaultItems(clientCapabilities) {
    ...
    const snippetSupport =
        clientCapabilities.textDocument?.completion?.completionItem
            ?.snippetSupport;
    const textEditDefault = Boolean(itemDefaults);
    const resolveDocumentation = resolveSupport?.includes('documentation');
    const resolveTextEditText = resolveSupport?.includes('textEditText');
    ...
    for (const [label, constant] of Object.entries(data.constants)) {
        ...
        if (textEditDefault && !resolveTextEditText) {
            item.textEditText = label;
        }
        if (textEditDefault) {
            fullItem.textEditText = label;
        }
    ...
    for (const [label, builtinFunction] of Object.entries(data.functions)) {
        ...
        const documentation = fromBuiltin(undefined, builtinFunction, useMarkdown);
        /** @type {CompletionItem} */
        const item = {
            label,
            kind: CompletionItemKind.Function,
